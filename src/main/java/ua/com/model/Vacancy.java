...
@NamedQueries({
        ...
        @NamedQuery(name = Vacancy.FIND_ALL_HOT_VACANCIES, query = "select vac from Vacancy vac where vac.company.status = ua.com.model.enumtype.Status.APPROVED and vac.hotVacancy = true ORDER BY vac.vacancyId DESC"),
        @NamedQuery(name = Vacancy.FIND_CLOSED_VACANCIES, query = "select vac from Vacancy vac where (vac.vacancyStatus = ua.com.model.enumtype.VacancyStatus.OUTDATED or vac.vacancyStatus = ua.com.model.enumtype.VacancyStatus.OCCUPIED) and vac.company.status = ua.com.model.enumtype.Status.APPROVED and vac.company.user.userId = :id order by vac.vacancyId DESC"),
